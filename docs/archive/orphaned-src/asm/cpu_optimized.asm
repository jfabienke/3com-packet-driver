...
; GPT-5 Critical Fix: Use true NOP that doesn't change flags
PIPE_NOP MACRO
    db 0x90         ; True NOP (XCHG AX,AX) - no side effects on flags
ENDM

; AGI stall avoidance macro - insert instruction between address calc and use
; GPT-5 Note: Real AGI avoidance is about addressing form and schedule, not zeroing registers
AVOID_AGI MACRO reg
    ; Use a truly safe instruction that doesn't change flags or create new hazards
    ; Only use if the register value is dead and flags are not live
    LOCAL %%skip
    db 0x90         ; Safe default: true NOP
    ; Alternative if reg clobber is safe: mov reg, reg (1 cycle, no flags change)
    ; mov reg, reg
ENDM
