...
#ifndef IO_DELAY_DEFINED
#define IO_DELAY_DEFINED
static void io_delay(void) {
    /* Standard I/O delay for slow ISA bus timing */
    /* Write to unused I/O port 0x80 for delay - Watcom compatible */
    outb(0x80, 0);
}
#endif

static void io_delay_us(uint16_t microseconds) {
...
static uint8_t reg_read8(uint16_t port) {
    uint8_t value = inb(port);
    io_delay();
    return value;
}

static void reg_write8(uint16_t port, uint8_t value) {
    outb(port, value);
    io_delay();
}

static uint16_t reg_read16(uint16_t port) {
    uint16_t value = inw(port);
    io_delay();
    return value;
}

static void reg_write16(uint16_t port, uint16_t value) {
    outw(port, value);
    io_delay();
}

static uint32_t reg_read32(uint16_t port) {
...
static void reg_write32(uint16_t port, uint32_t value) {
...
static void c509b_select_window(struct nic_context *ctx, uint8_t window) {
    reg_write16(ctx->io_base + _3C509B_COMMAND_REG, 
                _3C509B_CMD_SELECT_WINDOW | (window & 0x07));
}

/* Command register access */
static void c509b_write_command(struct nic_context *ctx, uint16_t command) {
    reg_write16(ctx->io_base + _3C509B_COMMAND_REG, command);
}

static uint16_t c509b_read_status(struct nic_context *ctx) {
    return reg_read16(ctx->io_base + _3C509B_STATUS_REG);
}

/* Window 0 - Configuration and EEPROM */
static uint16_t c509b_read_eeprom(struct nic_context *ctx, uint8_t offset) {
...
static uint16_t c509b_read_product_id(struct nic_context *ctx) {
    c509b_select_window(ctx, _3C509B_WINDOW_0);
    return reg_read16(ctx->io_base + _3C509B_W0_PRODUCT_ID);
}

/* Window 1 - Operating registers */
static uint16_t c509b_read_tx_status(struct nic_context *ctx) {
    c509b_select_window(ctx, _3C509B_WINDOW_1);
    return reg_read16(ctx->io_base + _3C509B_W1_TX_STATUS);
}

static uint16_t c509b_read_rx_status(struct nic_context *ctx) {
    c509b_select_window(ctx, _3C509B_WINDOW_1);
    return reg_read16(ctx->io_base + _3C509B_W1_RX_STATUS);
}

static void c509b_write_tx_data(struct nic_context *ctx, uint16_t data) {
    c509b_select_window(ctx, _3C509B_WINDOW_1);
    reg_write16(ctx->io_base + _3C509B_W1_TX_DATA, data);
}

static uint16_t c509b_read_rx_data(struct nic_context *ctx) {
    c509b_select_window(ctx, _3C509B_WINDOW_1);
    return reg_read16(ctx->io_base + _3C509B_W1_RX_DATA);
}

/* Window 2 - Station address */
static void c509b_write_station_address(struct nic_context *ctx, const uint8_t *mac) {
    int i;
    uint16_t addr_word;
    c509b_select_window(ctx, _3C509B_WINDOW_2);
    for (i = 0; i < 6; i += 2) {
        addr_word = mac[i] | (mac[i + 1] << 8);
        reg_write16(ctx->io_base + _3C509B_W2_STATION_ADDR + i, addr_word);
    }
}

static void c509b_read_station_address(struct nic_context *ctx, uint8_t *mac) {
    int i;
    uint16_t addr_word;
    c509b_select_window(ctx, _3C509B_WINDOW_2);
    for (i = 0; i < 6; i += 2) {
        addr_word = reg_read16(ctx->io_base + _3C509B_W2_STATION_ADDR + i);
...
static void c515_select_window(struct nic_context *ctx, uint8_t window) {
    reg_write16(ctx->io_base + _3C515_TX_COMMAND_REG, 
                _3C515_TX_CMD_SELECT_WINDOW | (window & 0x07));
}

/* Command and status register access */
static void c515_write_command(struct nic_context *ctx, uint16_t command) {
    reg_write16(ctx->io_base + _3C515_TX_COMMAND_REG, command);
}

static uint16_t c515_read_status(struct nic_context *ctx) {
    return reg_read16(ctx->io_base + _3C515_TX_STATUS_REG);
}

/* Window 0 - EEPROM access */
static uint16_t c515_read_eeprom(struct nic_context *ctx, uint8_t offset) {
    uint16_t timeout = 1000;
...
/* Window 1 - Operating registers */
static void c515_write_tx_data(struct nic_context *ctx, uint32_t data) {
    c515_select_window(ctx, _3C515_TX_WINDOW_1);
    reg_write32(ctx->io_base + _3C515_TX_W1_TX_DATA, data);
}

static uint32_t c515_read_rx_data(struct nic_context *ctx) {
    c515_select_window(ctx, _3C515_TX_WINDOW_1);
    return reg_read32(ctx->io_base + _3C515_TX_W1_RX_DATA);
}

static uint16_t c515_read_tx_status(struct nic_context *ctx) {
    c515_select_window(ctx, _3C515_TX_WINDOW_1);
    return reg_read16(ctx->io_base + _3C515_TX_W1_TX_STATUS);
}

static uint16_t c515_read_rx_status(struct nic_context *ctx) {
    c515_select_window(ctx, _3C515_TX_WINDOW_1);
    return reg_read16(ctx->io_base + _3C515_TX_W1_RX_STATUS);
}

/* Window 2 - Station address */
static void c515_write_station_address(struct nic_context *ctx, const uint8_t *mac) {
    int i;
    uint16_t addr_word;
...
static void c515_read_station_address(struct nic_context *ctx, uint8_t *mac) {
    int i;
    uint16_t addr_word;
    c515_select_window(ctx, _3C515_TX_WINDOW_2);
    for (i = 0; i < 6; i += 2) {
        addr_word = reg_read16(ctx->io_base + _3C515_TX_W2_STATION_ADDR + i);
        mac[i] = (uint8_t)(addr_word & 0xFF);
        mac[i + 1] = (uint8_t)(addr_word >> 8);
    }
}
...
static uint16_t c515_read_media_status(struct nic_context *ctx) {
    c515_select_window(ctx, _3C515_TX_WINDOW_4);
    return reg_read16(ctx->io_base + _3C515_TX_W4_MEDIA_STATUS);
}

static void c515_write_media_control(struct nic_context *ctx, uint16_t control) {
    c515_select_window(ctx, _3C515_TX_WINDOW_4);
    reg_write16(ctx->io_base + _3C515_TX_W4_MEDIA_CONTROL, control);
}

/* Window 7 - Bus master control */
static uint32_t c515_read_master_address(struct nic_context *ctx) {
    c515_select_window(ctx, _3C515_TX_WINDOW_7);
    return reg_read32(ctx->io_base + _3C515_TX_W7_MASTER_ADDRESS);
}

static void c515_write_master_address(struct nic_context *ctx, uint32_t address) {
    c515_select_window(ctx, _3C515_TX_WINDOW_7);
    reg_write32(ctx->io_base + _3C515_TX_W7_MASTER_ADDRESS, address);
}

static uint32_t c515_read_master_length(struct nic_context *ctx) {
    c515_select_window(ctx, _3C515_TX_WINDOW_7);
    return reg_read32(ctx->io_base + _3C515_TX_W7_MASTER_LENGTH);
}

static void c515_write_master_length(struct nic_context *ctx, uint32_t length) {
    c515_select_window(ctx, _3C515_TX_WINDOW_7);
    reg_write32(ctx->io_base + _3C515_TX_W7_MASTER_LENGTH, length);
}

static uint16_t c515_read_master_status(struct nic_context *ctx) {
    c515_select_window(ctx, _3C515_TX_WINDOW_7);
    return reg_read16(ctx->io_base + _3C515_TX_W7_MASTER_STATUS);
}

static void c515_write_master_control(struct nic_context *ctx, uint16_t control) {
    c515_select_window(ctx, _3C515_TX_WINDOW_7);
...
static void nic_select_window(struct nic_context *ctx, uint8_t window) {
...
static void nic_write_command(struct nic_context *ctx, uint16_t command) {
    if (ctx->nic_type == NIC_TYPE_3C509B) {
...
static uint16_t nic_read_status(struct nic_context *ctx) {
    if (ctx->nic_type == NIC_TYPE_3C509B) {
...
static void nic_write_station_address(struct nic_context *ctx, const uint8_t *mac) {
    if (ctx->nic_type == NIC_TYPE_3C509B) {
...
static void nic_read_station_address(struct nic_context *ctx, uint8_t *mac) {
    if (ctx->nic_type == NIC_TYPE_3C509B) {
...
static bool nic_validate_io_range(struct nic_context *ctx, uint16_t offset) {
    return (offset < ctx->io_range);
}

static bool nic_is_register_accessible(struct nic_context *ctx, uint16_t offset) {
    return (ctx->state != NIC_STATE_UNINITIALIZED && 
            nic_validate_io_range(ctx, offset));
}

/* Atomic register operations */
static uint16_t nic_read_modify_write16(struct nic_context *ctx, uint16_t offset, 
...
static void nic_set_bits16(struct nic_context *ctx, uint16_t offset, uint16_t bits) {
    nic_read_modify_write16(ctx, offset, bits, bits);
}

static void nic_clear_bits16(struct nic_context *ctx, uint16_t offset, uint16_t bits) {
