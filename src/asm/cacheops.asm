...
; CPU feature flag constants (must match cpudet.asm)
FEATURE_CLFLUSH     equ 100000h ; CLFLUSH instruction supported

; External reference to CPU optimization level (mapped via csym.inc)
...
global _mod_cacheops_header
_mod_cacheops_header:
    db  'PKTDRV', 0             ; +00  7 bytes: module signature
    db  1                       ; +07  1 byte:  major version
    db  0                       ; +08  1 byte:  minor version
    db  2                       ; +09  1 byte:  cpu_req (286+)
    db  0                       ; +0A  1 byte:  nic_type (0 = generic)
    db  1                       ; +0B  1 byte:  cap_flags (1 = MOD_CAP_CORE)
    dw  hot_end - hot_start     ; +0C  2 bytes: hot code size
    dw  patch_table_mod             ; +0E  2 bytes: offset to patch table
    dw  patch_table_mod_end - patch_table_mod  ; +10  2 bytes: patch table size
    dw  hot_start               ; +12  2 bytes: offset to hot_start
    dw  hot_end                 ; +14  2 bytes: offset to hot_end
    times 64 - ($ - _mod_cacheops_header) db 0  ; Pad to 64 bytes

section .text
hot_start:

global cache_clflush_line
global cache_clflush_safe
global cache_flush_range
global cache_wbinvd
global cache_wbinvd_safe
global cache_invd
global read_cr0_register
global write_cr0_register
global memory_fence
global memory_fence_after_clflush
global store_fence
global load_fence
global detect_cache_mode
...
cache_clflush_buffer:
    ...
    extern cpuid_available
    extern cpu_features

    cmp byte [cpuid_available], 0
    je .no_clflush

    test dword [cpu_features], FEATURE_CLFLUSH
    jz .no_clflush
    ...
    extern cache_line_size
    movzx ebx, byte [cache_line_size]
    test ebx, ebx
    jnz .has_line_size
...
cache_wbinvd_safe:
    ...
    extern cpu_is_486_plus
    cmp byte [cpu_is_486_plus], 0
    je .not_safe
...
;-----------------------------------------------------------------------------
memory_fence_after_clflush:
    push eax

    ; Check if SSE2 is available for MFENCE
    extern sse2_available
    extern cpuid_available

    cmp byte [sse2_available], 1
    je .use_mfence
...
.use_cpuid:
    ...
    push edx
    mov eax, 0
    db 0x0F, 0xA2       ; CPUID - encoded directly to avoid cpu 386 restriction
    pop edx
    pop ecx
    pop ebx
...
.done:
    pop eax
    ret
;; end memory_fence_after_clflush
...
size_done:
    ...
    pop ebx
    ret
;; end detect_cache_line_size_asm
