/**
 * @file pktops_rt.c
 * @brief Packet operations runtime functions (ROOT segment)
 *
 * 3Com Packet Driver - Support for 3C515-TX and 3C509B NICs
 *
 * This file contains performance-critical runtime functions for packet
 * transmission and reception that remain resident in conventional memory
 * (ROOT segment). These functions are called during interrupt handling
 * and normal packet TX/RX operations.
 *
 * Split from pktops.c - 2026-01-28 08:00:00 UTC
 *
 * Functions in this file:
 * - packet_send, packet_send_enhanced, packet_send_with_retry
 * - packet_receive, packet_receive_from_nic, packet_receive_with_recovery
 * - packet_isr_receive, packet_receive_process
 * - packet_process_tx_completions, packet_queue_tx_completion
 * - process_deferred_packets, packet_process_deferred_work
 * - Timer/tick helper functions
 * - Packet building and parsing utilities
 * - Multi-NIC routing and load balancing
 * - Queue TX/flush operations
 *
...
/* External globals defined in pktops_init.c */
extern int packet_ops_initialized;
extern packet_stats_t packet_statistics;
extern packet_queue_t g_packet_queues[];

/* External queue state defined in pktops_init.c */
extern struct {
    packet_queue_t tx_queues[4];
    packet_queue_t rx_queue;
    uint32_t queue_full_events;
    uint32_t backpressure_events;
    uint32_t priority_drops;
    uint32_t adaptive_resizes;
    bool flow_control_active;
    uint32_t last_queue_check;
} g_queue_state;

/* External bottom-half state defined in pktops_init.c */
extern struct bottom_half_state_s {
...
/* External XMS and deferred queue from pktops_init.c */
...
/* ========================================================================
 * Packet Transmission Functions (Hot Path)
 * ======================================================================== */
...
/* ========================================================================
 * Statistics and Health Monitoring (Runtime portions)
 * ======================================================================== */
