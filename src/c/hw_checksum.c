...
int hw_checksum_init(checksum_mode_t global_mode) {
    ...
    int result = hw_checksum_self_test();
...
int hw_checksum_configure_nic(nic_context_t *ctx, checksum_mode_t mode) {
    ...
    LOG_DEBUG("Configuring checksum mode %s for NIC %s", 
              hw_checksum_mode_to_string(mode), ctx->info->name);
    ...
    if (mode == CHECKSUM_MODE_HARDWARE) {
        if (!nic_has_capability(ctx, NIC_CAP_HWCSUM)) {
            LOG_WARNING("Hardware checksumming requested but not supported by %s", ctx->info->name);
            return HW_CHECKSUM_NOT_SUPPORTED;
        }
    }
    
    /* For 3C515-TX and 3C509B, force software mode */
    if (ctx->info->nic_type == NIC_TYPE_3C515_TX || ctx->info->nic_type == NIC_TYPE_3C509B) {
        if (mode == CHECKSUM_MODE_HARDWARE) {
            LOG_WARNING("Forcing software checksum mode for %s (no hardware support)", ctx->info->name);
            mode = CHECKSUM_MODE_SOFTWARE;
        } else if (mode == CHECKSUM_MODE_AUTO) {
            mode = CHECKSUM_MODE_SOFTWARE;
            LOG_DEBUG("Auto-selecting software checksum mode for %s", ctx->info->name);
        }
    }
...
uint32_t hw_checksum_detect_capabilities(nic_context_t *ctx) {
    ...
    if (ctx->info->nic_type == NIC_TYPE_3C515_TX || ctx->info->nic_type == NIC_TYPE_3C509B) {
        LOG_DEBUG("NIC %s: No hardware checksum capabilities (ISA generation)", ctx->info->name);
        return 0;
    }
    
    /* Check for hardware checksum capability flag */
    if (!nic_has_capability(ctx, NIC_CAP_HWCSUM)) {
        LOG_DEBUG("NIC %s: No hardware checksum capability flag set", ctx->info->name);
        return 0;
    }
    
    /* If we reach here, hardware might support checksumming */
    /* This is a placeholder for future NIC support */
    LOG_DEBUG("NIC %s: Hardware checksum capabilities detected (placeholder)", ctx->info->name);
...
int hw_checksum_tx_calculate(nic_context_t *ctx, uint8_t *packet, uint16_t length, uint32_t protocols) {
    ...
    uint8_t *ip_header = packet + ETH_HEADER_SIZE;
    uint16_t ip_header_len = (ip_header[IP_OFFSET_VERSION_IHL] & 0x0F) * 4;
    uint8_t ip_protocol = ip_header[IP_OFFSET_PROTOCOL];
    uint16_t ip_total_len = (ip_header[IP_OFFSET_TOTAL_LEN] << 8) | ip_header[IP_OFFSET_TOTAL_LEN + 1];
    
    int result = HW_CHECKSUM_SUCCESS;
    uint32_t start_time = packet_get_timestamp();
    ...
    uint8_t *transport_header = ip_header + ip_header_len;
    uint16_t transport_len = ip_total_len - ip_header_len;
    
    if (ip_protocol == IP_PROTO_TCP && (protocols & (1 << CHECKSUM_PROTO_TCP))) {
        checksum_context_t ctx_tcp = {0};
        ...
        uint32_t src_ip = *(uint32_t*)(ip_header + IP_OFFSET_SRC_IP);
        uint32_t dst_ip = *(uint32_t*)(ip_header + IP_OFFSET_DST_IP);
    ...
    /* Update statistics */
    uint32_t end_time = packet_get_timestamp();
    uint16_t calc_time = (end_time > start_time) ? (end_time - start_time) : 0;
...
int hw_checksum_calculate_ip(uint8_t *ip_header, uint16_t header_length) {
    ...
    /* Calculate checksum */
    uint16_t checksum = sw_checksum_internet(ip_header, header_length, 0);
...
int hw_checksum_calculate_tcp(checksum_context_t *ctx, uint8_t *tcp_header, uint16_t tcp_length) {
    ...
    uint16_t checksum = sw_checksum_internet(tcp_header, tcp_length, ctx->pseudo_header_sum);
...
int hw_checksum_calculate_udp(checksum_context_t *ctx, uint8_t *udp_header, uint16_t udp_length) {
    ...
    /* Calculate checksum including pseudo-header */
    uint16_t checksum = sw_checksum_internet(udp_header, udp_length, ctx->pseudo_header_sum);
...
int hw_checksum_rx_validate(nic_context_t *ctx, const uint8_t *packet, uint16_t length, uint32_t *result_mask) {
    ...
    *result_mask = 0;
    ...
    checksum_result_t ip_result = hw_checksum_validate_ip(ip_header, ip_header_len);
...
checksum_result_t hw_checksum_validate_tcp(const uint8_t *ip_header, const uint8_t *tcp_header, uint16_t tcp_length) {
    ...
    uint32_t src_ip = *(uint32_t*)(ip_header + IP_OFFSET_SRC_IP);
    uint32_t dst_ip = *(uint32_t*)(ip_header + IP_OFFSET_DST_IP);
    uint32_t pseudo_sum = sw_checksum_pseudo_header(src_ip, dst_ip, IP_PROTO_TCP, tcp_length);
    
    uint16_t checksum = sw_checksum_internet(tcp_header, tcp_length, pseudo_sum);
    return (checksum == 0) ? CHECKSUM_RESULT_VALID : CHECKSUM_RESULT_INVALID;
}
...
checksum_result_t hw_checksum_validate_udp(const uint8_t *ip_header, const uint8_t *udp_header, uint16_t udp_length) {
    ...
    uint16_t stored_checksum = (udp_header[UDP_OFFSET_CHECKSUM] << 8) | udp_header[UDP_OFFSET_CHECKSUM + 1];
    ...
    uint32_t src_ip = *(uint32_t*)(ip_header + IP_OFFSET_SRC_IP);
    uint32_t dst_ip = *(uint32_t*)(ip_header + IP_OFFSET_DST_IP);
    uint32_t pseudo_sum = sw_checksum_pseudo_header(src_ip, dst_ip, IP_PROTO_UDP, udp_length);
    
    uint16_t checksum = sw_checksum_internet(udp_header, udp_length, pseudo_sum);
    return (checksum == 0) ? CHECKSUM_RESULT_VALID : CHECKSUM_RESULT_INVALID;
}
...
int hw_checksum_self_test(void) {
    /* Test data: Simple IP packet with known checksums */
    ...
    int result = hw_checksum_calculate_ip(test_ip_header, 20);
    if (result != HW_CHECKSUM_SUCCESS) {
        LOG_ERROR("IP checksum calculation failed");
        return result;
    }
    
    uint16_t calculated_checksum = (test_ip_header[10] << 8) | test_ip_header[11];
    ...
    checksum_result_t validation_result = hw_checksum_validate_ip(test_ip_header, 20);
