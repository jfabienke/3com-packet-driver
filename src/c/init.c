...
/* #include "smc_safety_patches.h" */ /* SMC safety detection and patching - file not found */
...
int detect_cpu_type(void) {
    ...
    log_info("CPU detected: %s", cpu_type_to_string(g_cpu_info.cpu_type));
    log_info("CPU features: 0x%08X", g_cpu_info.features);
    
    return (int)g_cpu_info.cpu_type;
}
...
int hardware_init_all(const config_t *config) {
    ...
    detected_3c509b = nic_detect_3c509b(detect_info, MAX_NICS);
    ...
    if (g_cpu_info.cpu_type < CPU_TYPE_80286) {
        ...
        cpu_supports_busmaster = (g_cpu_info.cpu_type >= CPU_TYPE_80286) &&
        ...
        if (!cpu_supports_busmaster && config->busmaster != BUSMASTER_OFF) {
            ...
            detected_3c515 = nic_detect_3c515(detect_info, MAX_NICS - num_nics);
            if (detected_3c515 > 0) {
    ...
    if (g_cpu_info.cpu_type < CPU_TYPE_80286) {
        ...
        if (config->pci != PCI_DISABLED && is_pci_available()) {
            pci_nics = detect_and_init_pci_nics(config, MAX_NICS - num_nics);
            if (pci_nics > 0) {
    ...
    if (num_nics > 0) {
        ...
        log_info("CPU: %s (features: 0x%08X)", 
                cpu_type_to_string(g_cpu_info.cpu_type), g_cpu_info.features);
...
int init_complete_sequence(const config_t *config) {
    int result = 0;
    /* C89: extern declarations must be at block start */
    ...
    log_info("  Environment: %s", init_state.platform.environment_desc);
    log_info("  VDS available: %s", init_state.platform.requires_vds ? "Required" : "Optional");
