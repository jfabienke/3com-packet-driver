...
static uint32_t fc_quanta_to_ms(uint16_t quanta, uint32_t link_speed_mbps) {
    ...
    num = (uint32_t)quanta * 512UL;
    den = link_speed_mbps * 1000UL;
    ms = (den > 0) ? (num + den - 1) / den : 0; /* round up */
    /* Safety cap */
...
bool fc_simple_should_pause(int nic_index) {
    ...
    now = stats_get_timestamp();
    pause_timer_active = (now < st->pause_until_ms);
...
void fc_simple_wait_for_resume(int nic_index, uint32_t pause_ms) {
    ...
    start = stats_get_timestamp();
    deadline = start + pause_ms;
    while (stats_get_timestamp() < deadline) {
        /* Tight spin; keep short to avoid blocking long periods */
        /* Break early if fc_simple_should_pause() clears */
        if (!fc_simple_should_pause(nic_index)) break;
    }
}
...
int fc_simple_process_rx(int nic_index,
    ...
    opcode = ((uint16_t)packet[14] << 8) | packet[15];
    if (opcode != PAUSE_FRAME_OPCODE) return 0; /* Ignore other MAC control */

    pause_quanta = ((uint16_t)packet[16] << 8) | packet[17];

    /* Calculate milliseconds from quanta based on NIC speed */
    nic = hardware_get_nic(nic_index);
    link_speed = (nic && nic->speed > 0) ? (uint32_t)nic->speed : 10UL; /* Mbps */
    pause_ms = fc_quanta_to_ms(pause_quanta, link_speed);
...
#ifndef NO_LOGGING
    log_debug("Flow control: PAUSE quanta=%u (~%lums) on NIC %d",
              (unsigned)pause_quanta, (unsigned long)pause_ms, nic_index);
#endif
