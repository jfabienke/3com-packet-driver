 ...
 *
 * Renamed from smcpat.c to avoid .obj collision with smcpat.asm
 *
...
/* Patch template stubs */
const uint8_t patch_template_rep_movsw[] = {0};
const uint8_t patch_template_rep_movsd[] = {0};
const uint8_t patch_template_pusha_popa[] = {0};
const uint8_t patch_template_string_io[] = {0};
const uint8_t patch_template_sizes[] = {0};

/* Assembly function stubs - smcpat.asm exports these without trailing underscore
 * (ASM-to-ASM convention), but C expects trailing underscore. Provide static
 * stubs here so the C orchestration layer links standalone. The real ASM
 * implementations are called from other ASM modules directly. */
static void flush_instruction_prefetch(void) {}
static void flush_prefetch_at_address(void FAR *address) { (void)address; }
static void asm_flush_prefetch_near_jump(void) {}
static int asm_atomic_patch_bytes(void FAR *target, const void FAR *patch, uint8_t size) {
    (void)target; (void)patch; (void)size;
    memcpy(target, patch, size);
    return 0;
}
static void asm_save_interrupt_state(void) {}
static void asm_restore_interrupt_state(void) {}
...
patch_application_result_t apply_patches_atomic(void) {
    ...
    if (result.patches_failed == 0) {
        ...
        strcpy(result.error_message, "Partial: some applied, some failed");
    } else {
        result.status = PATCH_STATUS_FAILED;
 ...
 */
static int prepare_endian_patch(patch_site_t *site, cpu_type_t cpu_type) {
    /* Stub: endian patch preparation requires ASM swap functions from flowrt.asm
     * which export without trailing underscore (ASM-to-ASM convention).
     * The real patching is done via the ASM-side SMC framework. */
    (void)site;
    (void)cpu_type;
    return SUCCESS;
}
 ...
 * Functions declared in smcpat.h but not yet fully implemented.
 * These provide minimal default behavior to satisfy the linker.
 * ============================================================ */
